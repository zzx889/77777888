...
Notification:Notify(
    {Title = "欢迎使用脚本", Description = "sy：917813"},
...
local Window = WindUI:CreateWindow({
    Title = '<font color="#FF3333">妥</font><font color="#FF9933">协</font><font color="#FFFF33">脚</font><font color="#33FF33">本</font>    ',  
    Icon = "badge-check",
    Author = "作者:妥协",
...
local LanguageService = {
    ...
    Translations = {
        -- 基础界面文本
        ["妥协脚本"] = {en = "Black & White Script", ja = "妥協する"},
        ["作者:妥协"] = {en = "Author: Unknown Quantity B&W", ja = "作者: 妥协"},
        ["欢迎使用脚本"] = {en = "Welcome to B&W Script", ja = "妥協する"},
        ["正在加载资源..."] = {en = "Loading resources...", ja = "ちょっと待ってください、お兄ちゃん..."},
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if localRootPart and targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            Notify("妥协脚本", "已经传送到玩家身边", 5)
        else
            Notify("妥协脚本", "无法传送 原因: 玩家已消失", 5)
        end
    end
})
...
local function ThrowPlayer(targetPlayer)
    ...
    if not localCharacter or not localHumanoid or not localRootPart or not targetCharacter then
        SendNotification("妥协脚本", "无法甩飞: 角色不存在", 5)
        return
    end
    ...
    if targetHumanoid and targetHumanoid.Sit then
        SendNotification("妥协脚本", "玩家正在坐下", 5)
        return
    end
    ...
    elseif targetAccessory and accessoryHandle then
        ...
        SendNotification("妥协脚本", "无法甩飞目标", 5)
        return
    end
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if foundPlayer == "all" then
            ...
            SendNotification("妥协脚本", "正在甩飞所有玩家", 5)
        elseif foundPlayer then
            if foundPlayer.UserId == 1414978355 then
                SendNotification("妥协脚本", "检测到特殊玩家，已停止", 5)
                return
            end
            ThrowPlayer(foundPlayer)
            SendNotification("妥协脚本", "正在甩飞玩家", 5)
        else
            SendNotification("妥协脚本", "未找到玩家", 5)
        end
    end
})
...
local Paragraph = Tab0:Paragraph({
    ...
    -- 视觉设置
    Image = "https://i.ibb.co/KzpGVsQR/153941.png", -- 背景图片
    ImageSize = 30, -- 图片显示尺寸
    Thumbnail = "", -- 缩略图（留空）
